...
import { MantineThemeOverride, MantineColorsTuple, DEFAULT_THEME, MantineColorShade } from '@mantine/core';
...
class ThemeManager {
    ...
    constructor(theme: MantineThemeOverride, stateFunction: (theme: MantineThemeOverride) => void) {
        this.schemeDependentPrimaryShade = theme.primaryShade && typeof theme.primaryShade === 'object' ? true : false;
        this.theme = theme;
    ...
    setColorFromString(key: string, value: string) {
        this.theme.colors ? this.theme.colors[key] = generateShades(value) as unknown as MantineColorsTuple : this.theme.colors = { [key]: generateShades(value) as unknown as MantineColorsTuple };
        this.commitChanges();
    }
    ...
    getPrimaryColor() {
        return this.theme.primaryColor ? this.theme.primaryColor : DEFAULT_THEME.primaryColor;
    }
    ...
    getDefaultGradient() {
        return this.theme.defaultGradient ? this.theme.defaultGradient : DEFAULT_THEME.defaultGradient;
    }

    getGradientFrom() {
        return this.theme.defaultGradient ? this.theme.defaultGradient.from : DEFAULT_THEME.defaultGradient.from;
    }
    ...
    setGradientFrom(value: string) {
        this.theme.defaultGradient ? this.theme.defaultGradient.from = value : this.theme.defaultGradient = { from: value, to: this.getGradientTo() };
        this.commitChanges();
    }

    getGradientTo() {
        return this.theme.defaultGradient ? this.theme.defaultGradient.to : DEFAULT_THEME.defaultGradient.to;
    }

    setGradientTo(value: string) {
        this.theme.defaultGradient ? this.theme.defaultGradient.to = value : this.theme.defaultGradient = { from: this.getGradientFrom(), to: value };
        this.commitChanges();
    }
    ...
    getCustomColors() {
        ...
        for (let color in this.theme.colors) {
            if (this.theme.colors[color] && !DEFAULT_THEME.colors[color]) {
    ...
    getMantineColors() {
        ...
        for (let color in DEFAULT_THEME.colors) {
            ...
            if (this.theme.colors[color] && DEFAULT_THEME.colors[color]) {
                ...
                mantineColors.set(color, DEFAULT_THEME.colors[color]);
            }
        }
    ...
    getAllColors() {
        return this.theme.colors ? this.theme.colors : DEFAULT_THEME.colors;
    }

    getAllMainColorArray() {
        return Array.from(Object.values(this.getAllColors() || {})).map((color) => color ? color[5] : "#000");
    }
    ...
    getDefaultRadius() {
        return this.theme.defaultRadius ? this.theme.defaultRadius : DEFAULT_THEME.defaultRadius;
    }

    getWhite() {
        return this.theme.white ? this.theme.white : DEFAULT_THEME.white;
    }
    ...
    getBlack() {
        return this.theme.black ? this.theme.black : DEFAULT_THEME.black;
    }
    ...
    getAutoContrast() {
        return this.theme.autoContrast ? this.theme.autoContrast : DEFAULT_THEME.autoContrast;
    }
    ...
    getLuminanceThreshold() {
        return this.theme.luminanceThreshold ? this.theme.luminanceThreshold : DEFAULT_THEME.luminanceThreshold;
    }
