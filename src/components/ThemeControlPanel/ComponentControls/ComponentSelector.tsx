...
const ComponentSelector: React.FC<{
    ...
    return (
        <Select
            label={t('components.selectComponent')}
            placeholder={t('components.choosePlaceholder')}
            value={value}
            onChange={onChange}
            w="100%"
            searchable
            data={Object.entries(CATEGORIZED_COMPONENTS).map(([category, components]) => ({
                group: t(`components.categories.${category}`, {defaultValue: category.charAt(0).toUpperCase() + category.slice(1)}),
